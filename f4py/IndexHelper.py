...
#                compressor = zstandard.ZstdCompressor(level=self.__compression_level)
    ...
    def filter(f4_file_path, compression_level, index_column, index_column_type, row_indices, fltr):
        ...
        if index_column_type == "i":
            if isinstance(fltr, f4py.StringEqualsFilter):
                return f4py.IdentifierIndexer(f4_file_path, compression_level).filter(index_file_path, row_indices, fltr)
            else:
                raise Exception("TODO: Not yet supported.")

        if index_column_type == "c":
            return f4py.CategoricalIndexer(f4_file_path, compression_level).filter(index_file_path, row_indices, fltr)

        if isinstance(fltr, f4py.NumericFilter):
            return f4py.NumericIndexer(f4_file_path, compression_level).filter(index_file_path, row_indices, fltr)
        else:
            raise Exception("TODO: Not yet supported.")
