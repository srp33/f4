...
class IndexHelper:
    ...
    def save_indices(f4_file_path, index_columns, compression_level=None, verbose=False):
        for index_column in index_columns:
            f4py.IndexHelper.save_index(f4_file_path, index_column, compression_level=compression_level, verbose=verbose)
    ...
    def _binary_index_search(parser, line_length, value_coords, data_file_handle, value_to_find, l, r):
        ...
        if mid_value == value_to_find:
            ...
            return IndexHelper._binary_index_search(parser, line_length, value_coords, data_file_handle, value_to_find, l, mid-1)
        else:
            # Else the element can only be present in right subarray
            return IndexHelper._binary_index_search(parser, line_length, value_coords, data_file_handle, value_to_find, mid+1, r)
    ...
    def _find_positions_g(index_parser, line_length, value_coords, data_file_handle, filter_value, end_index, all_true_operator, all_false_operator, conversion_function):
        ...
        matching_position = f4py.IndexHelper._search(index_parser, line_length, value_coords, data_file_handle, filter_value, 0, end_index, all_false_operator, conversion_function)
    ...
    def _find_positions_l(index_parser, line_length, value_coords, data_file_handle, filter_value, end_index, all_true_operator, all_false_operator, conversion_function):
        ...
        matching_position = f4py.IndexHelper._search(index_parser, line_length, value_coords, data_file_handle, filter_value, 0, end_index, all_true_operator, conversion_function)
    ...
    def _retrieve_matching_row_indices(index_parser, position_coords, positions, num_processes):
        ...
        if num_processes == 1 or num_indices < 100:
            return f4py.IndexHelper._find_matching_row_indices(index_parser.data_file_path, position_coords, positions)
        else:
            ...
            return set(chain.from_iterable(Parallel(n_jobs = num_processes)(delayed(f4py.IndexHelper._find_matching_row_indices)(index_parser.data_file_path, position_coords, position_chunk) for position_chunk in position_chunks)))
